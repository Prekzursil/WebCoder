...
const ProblemFormPage: React.FC = () => {
	...
	const [defaultMemoryLimitKb, setDefaultMemoryLimitKb] =
		useState<number>(262144);
	...
	const [comparisonMode, setComparisonMode] =
		useState<string>("LINES_STRIP_EXACT");
	const [floatComparisonEpsilon, setFloatComparisonEpsilon] = useState<
		number | null
	>(1e-6);
			...
			ProblemService.getProblemDetail(problemId)
				.then((response: any) => {
					...
					setFloatComparisonEpsilon(
						data.float_comparison_epsilon === null
							? null
							: data.float_comparison_epsilon || 1e-6,
					);
					...
					setTestCases(
						data.test_cases?.map(
							(tc: TestCaseType): TestCaseUIManaged => ({
								...tc,
				...
				.catch(() => {
					setFormError(
						t(
							"error_loading_problem_for_edit",
	...
	const handleAddTestCase = async () => {
		if (!newTestCaseInput.trim() || !newTestCaseOutput.trim()) {
			alert(
				t(
					"test_case_input_output_required",
					"Test case input and output cannot be empty.",
				),
			);
			return;
		}
		...
		if (isEditMode && problemId && tcData.problem !== null) {
			try {
				const response: any = await TestCaseService.createTestCase(
					tcData as TestCaseType,
				);
				const createdTC = response.data;
				setTestCases((prev) => [
					...prev,
					{ ...createdTC, local_id: createdTC.id.toString() },
				]);
				setFormMessage(
					t("test_case_added_successfully", "Test case added successfully!"),
				);
			} catch (err: any) {
				setFormError(
					err.message ||
						t("error_adding_test_case", "Failed to add test case."),
				);
				return;
			}
		} else {
	...
	const handleRemoveTestCase = async (tcToRemove: TestCaseUIManaged) => {
		...
		if (tcToRemove.id) {
			try {
				...
				setFormMessage(
					t(
						"test_case_removed_successfully",
				...
				setFormError(
					err.message ||
						t("error_removing_test_case", "Failed to remove test case."),
				);
			}
		} else if (tcToRemove.local_id) {
			setTestCases((prev) =>
				prev.filter((tc) => tc.local_id !== tcToRemove.local_id),
			);
		}
	};
	...
	const handleSubmit = async (event: FormEvent) => {
		...
		if (!auth.token) {
			setFormError(
				t("error_auth_required_problem_form", "Authentication required."),
			);
			return;
		}
		...
		const problemDataPayload: Partial<ProblemType> = {
			...
			float_comparison_epsilon:
				comparisonMode === "FLOAT_PRECISE" ? floatComparisonEpsilon : null,
			checker_code: comparisonMode === "CUSTOM_CHECKER" ? checkerCode : null,
			checker_language:
				comparisonMode === "CUSTOM_CHECKER" ? checkerLanguage : null,
		...
		try {
			if (isEditMode && problemId) {
				...
				setFormMessage(
					t("problem_updated_successfully", "Problem updated successfully!"),
				);
			} else {
				const response: any = await ProblemService.createProblem(
					problemDataPayload as ProblemType,
				);
				const newProblem = response.data;
				setFormMessage(
					t("problem_created_successfully", "Problem created successfully!"),
				);
				if (newProblem.id) {
			...
			if (err.response?.data && typeof err.response.data === "object") {
				setFormError(
					Object.entries(err.response.data)
						...
						.join("; ") || t("error_saving_problem", "Failed to save problem."),
				);
			} else {
				setFormError(
					err.message || t("error_saving_problem", "Failed to save problem."),
				);
			}
		} finally {
	...
	return (
		<div style={{ maxWidth: "800px", margin: "0 auto", padding: "20px" }}>
			<h2>
				{isEditMode
					? t("edit_problem_header", "Edit Problem")
					: t("create_problem_header", "Create New Problem")}
			</h2>
			...
			<form onSubmit={handleSubmit}>
				<div style={sectionStyle}>
					<h4>
						{t(
							"problem_form_section_content",
							"Problem Content & Identification",
						)}
					</h4>
					<div style={fieldStyle}>
						<label htmlFor="title_en" style={labelStyle}>
							{t("problem_form_title_en", "Title (English)")}:
						</label>
						<input
							type="text"
							id="title_en"
					...
					<div style={fieldStyle}>
						<label htmlFor="title_ro" style={labelStyle}>
							{t("problem_form_title_ro", "Title (Romanian)")}:
						</label>
						<input
							type="text"
							id="title_ro"
					...
					<div style={fieldStyle}>
						<label htmlFor="statement_en" style={labelStyle}>
							{t("problem_form_statement_en", "Statement (English)")}:
						</label>
						<textarea
							id="statement_en"
							value={statementEn}
					...
					<div style={fieldStyle}>
						<label htmlFor="statement_ro" style={labelStyle}>
							{t("problem_form_statement_ro", "Statement (Romanian)")}:
						</label>
						<textarea
							id="statement_ro"
							value={statementRo}
							onChange={(e) => setStatementRo(e.target.value)}
							rows={10}
							required
							style={textAreaStyle}
						/>
				...
				<div style={sectionStyle}>
					...
					<div style={fieldStyle}>
						<label htmlFor="difficulty" style={labelStyle}>
							{t("problem_form_difficulty", "Difficulty")}:
						</label>
						<select
							id="difficulty"
							value={difficulty}
					...
					<div style={fieldStyle}>
						<label htmlFor="time_limit" style={labelStyle}>
							{t("problem_form_time_limit_ms", "Time Limit (ms)")}:
						</label>
						<input
							type="number"
							id="time_limit"
							...
							onChange={(e) =>
								setDefaultTimeLimitMs(parseInt(e.target.value, 10))
							}
							required
							style={inputStyle}
						/>
					...
					<div style={fieldStyle}>
						<label htmlFor="memory_limit" style={labelStyle}>
							{t("problem_form_memory_limit_kb", "Memory Limit (KB)")}:
						</label>
						<input
							type="number"
							id="memory_limit"
							...
							onChange={(e) =>
								setDefaultMemoryLimitKb(parseInt(e.target.value, 10))
							}
							required
							style={inputStyle}
						/>
					...
					<div style={fieldStyle}>
						<label style={labelStyle}>
							{t("problem_form_allowed_languages", "Allowed Languages")}:
						</label>
						{["python3", "cpp17", "java11"].map((lang) => (
							<span key={lang} style={checkboxContainerStyle}>
								<input
									type="checkbox"
									id={`lang-${lang}`}
									value={lang}
									...
									onChange={(e) => {
										if (e.target.checked) {
											setAllowedLanguages((prev) => [...prev, lang]);
										} else {
								...
								/>
								<label htmlFor={`lang-${lang}`} style={checkboxLabelStyle}>
									{lang}
								</label>
							</span>
						))}
				...
				<div style={sectionStyle}>
					<h4>
						{t(
							"problem_form_section_advanced_config",
							"Advanced Configuration",
						)}
					</h4>
					<div style={fieldStyle}>
						<label htmlFor="comparison_mode" style={labelStyle}>
							{t("problem_form_comparison_mode", "Comparison Mode")}:
						</label>
						<select
							id="comparison_mode"
							...
							style={inputStyle}
						>
							<option value="EXACT">{t("comparison_mode_exact")}</option>
							<option value="STRIP_EXACT">
								{t("comparison_mode_strip_exact")}
							</option>
							<option value="LINES_STRIP_EXACT">
								{t("comparison_mode_lines_strip_exact")}
							</option>
							<option value="FLOAT_PRECISE">
								{t("comparison_mode_float_precise")}
							</option>
							<option value="CUSTOM_CHECKER">
								{t("comparison_mode_custom_checker")}
							</option>
						</select>
					</div>
					{comparisonMode === "FLOAT_PRECISE" && (
						<div style={fieldStyle}>
							<label htmlFor="float_epsilon" style={labelStyle}>
								{t("problem_form_float_epsilon")}:
							</label>
							<input
								type="number"
								step="any"
								id="float_epsilon"
								value={
									floatComparisonEpsilon === null ? "" : floatComparisonEpsilon
								}
								onChange={(e) =>
									setFloatComparisonEpsilon(
					...
					{comparisonMode === "CUSTOM_CHECKER" && (
						<>
							<div style={fieldStyle}>
								<label htmlFor="checker_language" style={labelStyle}>
									{t("problem_form_checker_language")}:
								</label>
								<select
									id="checker_language"
									value={checkerLanguage || ""}
									onChange={(e) => setCheckerLanguage(e.target.value || null)}
									style={inputStyle}
								>
									<option value="">{t("select_checker_language")}</option>
									<option value="python3">Python 3</option>
									<option value="cpp17">C++17</option>
								</select>
							</div>
							<div style={fieldStyle}>
								<label htmlFor="checker_code" style={labelStyle}>
									{t("problem_form_checker_code")}:
								</label>
								<textarea
									id="checker_code"
									value={checkerCode || ""}
									onChange={(e) => setCheckerCode(e.target.value || null)}
									rows={10}
									style={textAreaStyle}
								/>
							</div>
				...
				<div style={sectionStyle}>
					...
					<div style={fieldStyle}>
						{availableTags.map((tag) => (
							<span key={tag.id} style={checkboxContainerStyle}>
								<input
									type="checkbox"
									id={`tag-${tag.id}`}
									value={tag.id}
								...
								/>
								<label htmlFor={`tag-${tag.id}`} style={checkboxLabelStyle}>
									{tag.name_i18n[i18n.language] || tag.name_i18n.en || tag.slug}
								</label>
							</span>
						))}
					</div>
				...
				<div style={sectionStyle}>
					...
					{testCases.map((tc: TestCaseUIManaged, index: number) => (
						<div
							...
							style={{
								...
								padding: "10px",
								marginBottom: "10px",
								backgroundColor: tc.is_sample ? "#f0f8ff" : "transparent",
							}}
						>
							<h4>
								{t("test_case_label", "Test Case")} {index + 1}{" "}
								{tc.is_sample && `(${t("sample_label", "Sample")})`}
							</h4>
							<p>
								<strong>{t("points_label", "Points")}:</strong> {tc.points}
							</p>
							<div>
								<label style={labelStyle}>
									{t("input_data_label", "Input")}:
								</label>
								<pre
									style={{
							...
							<div>
								<label style={labelStyle}>
									{t("expected_output_label", "Output")}:
								</label>
								<pre
									style={{
										backgroundColor: "#f9f9f9",
							...
							>
								{t("remove_test_case_button", "Remove")}
							</button>
						</div>
					))}
					<div
					...
					>
						...
						<div style={fieldStyle}>
							<label htmlFor="new_tc_input" style={labelStyle}>
								{t("input_data_label", "Input Data")}:
							</label>
							<textarea
								id="new_tc_input"
						...
						<div style={fieldStyle}>
							<label htmlFor="new_tc_output" style={labelStyle}>
								{t("expected_output_label", "Expected Output Data")}:
							</label>
							<textarea
								id="new_tc_output"
						...
						<div style={fieldStyle}>
							<label htmlFor="new_tc_points" style={labelStyle}>
							...
							<input
								...
								onChange={(e) =>
									setNewTestCasePoints(parseInt(e.target.value, 10) || 0)
								}
								style={inputStyle}
							/>
						</div>
						<div style={fieldStyle}>
							<label htmlFor="new_tc_is_sample" style={labelStyle}>
								{t("is_sample_label", "Is Sample?")}:
							</label>
							<input
								type="checkbox"
								id="new_tc_is_sample"
								checked={newTestCaseIsSample}
								onChange={(e) => setNewTestCaseIsSample(e.target.checked)}
						...
						>
							{t("add_test_case_button", "Add Test Case")}
						</button>
					</div>
				</div>
				...
				>
					{isEditMode
						...
						: t("create_problem_button", "Create Problem")}
				</button>
			</form>
		</div>
	);
