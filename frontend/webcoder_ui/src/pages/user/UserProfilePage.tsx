...
const UserProfilePage: React.FC = () => {
	...
	const handleChangePassword = async (e: FormEvent) => {
		...
		try {
			...
			setMessage(
				t("password_change_successful", "Password changed successfully."),
			);
			setCurrentPassword("");
			setNewPassword("");
			...
			if (err.response?.data && typeof err.response.data === "object") {
				setError(
					Object.entries(err.response.data)
						...
						.join("; ") ||
						t("password_change_failed", "Password change failed."),
				);
			} else {
				setError(
					err.message || t("password_change_failed", "Password change failed."),
				);
			}
		} finally {
	...
	const isOwnProfile =
		!userId || (auth.user && auth.user.id.toString() === userId);
	...
	return (
		<Container maxWidth="md">
			<Box sx={{ my: 4 }}>
				...
				<Paper sx={{ p: 2, mb: 4 }}>
					<Typography>
						<strong>{t("username_label", "Username")}:</strong>{" "}
