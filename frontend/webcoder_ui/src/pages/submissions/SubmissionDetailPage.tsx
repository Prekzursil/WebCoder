...
const SubmissionDetailPage: React.FC = () => {
  ...
  const fetchDetails = async (currentSubmissionId: string) => {
    try {
      ...
      if (
        data &&
        (data.verdict === 'PENDING' || data.verdict === 'COMPILING' || data.verdict === 'RUNNING')
      ) {
      ...
      setError(
        err.message || t('error_loading_submission_detail', 'Failed to load submission details.'),
      );
      if (isPolling) setIsPolling(false);
    }
  };
  ...
  useEffect(() => {
    if (submissionId && auth.token) {
      ...
      setError(
        t('error_auth_required_submission', 'Authentication required to view submission details.'),
      );
      setLoading(false);
    }
  ...
  const renderTestResult = (result: SubmissionTestResultType) => (
    ...
    >
      ...
      {result.error_output &&
        (result.verdict === 'RE' || result.verdict === 'CE' || result.verdict === 'IE') && (
          <>
            <p>
  ...
  return (
    <div>
      ...
      {submission.execution_time_ms !== null && (
        <p>
          <strong>{t('execution_time_label', 'Execution Time (Overall)')}:</strong>{' '}
