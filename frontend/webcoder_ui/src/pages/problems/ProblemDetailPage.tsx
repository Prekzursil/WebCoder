...
const ProblemDetailPage: React.FC = () => {
	...
	const handleActualSubmit = async () => {
		if (!auth.token || !problemId) {
			setSubmissionError(
				t(
					"error_not_logged_in_or_no_problem",
		...
		if (!selectedLanguage) {
			setSubmissionError(
				t("error_select_language", "Please select a language."),
			);
			setIsModalOpen(false);
			return;
		}
		...
		try {
			...
			setSubmissionStatus(
				t("submission_successful_pending", { id: response.data.id }),
			);
			setCode("");
		} catch (err: any) {
			setSubmissionError(
				err.message || t("submission_failed_error", "Submission failed."),
			);
			setSubmissionStatus(null);
		} finally {
	...
	const problemTitle =
		problem.title_i18n[currentLang] ||
		problem.title_i18n.en ||
		`${t("problem_id_label", "Problem ID")}: ${problem.id}`;
	const problemStatement =
		problem.statement_i18n?.[currentLang] || problem.statement_i18n?.en || "";
	...
	return (
		<div>
				...
				<>
					...
					{sampleTestCases.map((tc, index) => (
						...
						>
							<p>
								<strong>
									{t("sample_input_label", "Sample Input")} {index + 1}:
								</strong>
							</p>
							<pre>{tc.input_data}</pre>
							<p>
								<strong>
									{t("sample_output_label", "Sample Output")} {index + 1}:
								</strong>
							</p>
			...
			<ConfirmationModal
				...
				message={t("confirm_submission_message", {
					language: selectedLanguage,
