...
class SubmissionAdmin(admin.ModelAdmin):
    ...
    def user_link(self, obj):
        ...
        if obj.user:
            link = reverse(
                "admin:users_user_change", args=[obj.user.id]
            )  # Assumes app_label is 'users'
    ...
    def problem_link(self, obj):
        ...
        if obj.problem:
            link = reverse(
                "admin:problems_problem_change", args=[obj.problem.id]
            )  # Assumes app_label is 'problems'
            # Display problem title (e.g., English) if available
            problem_title = (
                obj.problem.title_i18n.get("en", f"ID: {obj.problem.id}")
                if isinstance(obj.problem.title_i18n, dict)
                else f"ID: {obj.problem.id}"
            )
