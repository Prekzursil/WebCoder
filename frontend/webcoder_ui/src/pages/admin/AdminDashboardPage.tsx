		...
		const fetchUsers = async () => {
			if (auth.token) {
				try {
					...
					setError(
						err.message ||
							t("errors.failed_to_fetch_users", "Failed to fetch users."),
					);
				} finally {
					setLoading(false);
				}
		...
		if (auth.token) {
			try {
				...
				setUsers(
					users.map((user) =>
						user.id === userId
							? { ...user, role: newRole as AdminUserType["role"] }
							: user,
					),
				);
				toast.success(
					t(
						"admin_dashboard.update_role_success",
						"Role updated successfully.",
					),
				);
			} catch (err: any) {
				toast.error(
					t("errors.failed_to_update_role", "Failed to update role: ") +
						err.message,
				);
			}
		}
	...
	const handleStatusChange = async (userId: number, isActive: boolean) => {
		if (auth.token) {
			try {
				...
				toast.success(
					t(
						"admin_dashboard.update_status_success",
						"Status updated successfully.",
					),
				);
			} catch (err: any) {
				toast.error(
					t("errors.failed_to_update_status", "Failed to update status: ") +
						err.message,
				);
			}
		}
