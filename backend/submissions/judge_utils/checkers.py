...
def run_custom_checker(
    ...
    if checker_language == "python3":
        ...
        with open(checker_script_file, "w") as f:
            f.write(checker_code)
        checker_executable_path_str, compile_ok, compiler_out = (
            str(checker_script_file),
            True,
            "Checker is Python.",
        )
        ...
        executable_file_in_container, executable_file_on_host = (
            "checker_program",
            checker_submission_dir / "checker_program",
        )
        ...
        checker_compile_cmd = [
            ...
            "-v",
            f"{abs_checker_dir}:/app",
            "-w",
            "/app",
            "--network",
            "none",
            docker_image_checker_compile,
            "g++",
            "checker.cpp",
            "-o",
            executable_file_in_container,
            "-std=c++17",
            "-O2",
        ]
        try:
            result = subprocess.run(
                checker_compile_cmd, capture_output=True, text=True, timeout=30
            )
            compiler_out = f"Checker Compile STDOUT:\n{result.stdout}\nChecker Compile STDERR:\n{result.stderr}"
            if result.returncode == 0 and executable_file_on_host.exists():
                compile_ok, checker_executable_path_str = (
                    True,
                    str(executable_file_on_host),
                )
    ...
    current_checker_run_command_args = list(
        checker_run_command_args
    )  # Make a copy to avoid modifying the template
    current_checker_run_command_args.extend(
        [container_input_path, container_user_out_path, container_ans_path]
    )

    checker_time_limit_s, checker_mem_limit_kb = 10, 262144
    time_format_string_checker = "%U %S %M %x %P %e"

    command_to_time_checker = [
        "/usr/bin/time",
        "-f",
        time_format_string_checker,
    ] + current_checker_run_command_args
    command_for_docker_checker = [
        "timeout",
        "--signal=SIGKILL",
        f"{checker_time_limit_s + 1:.2f}s",
    ] + command_to_time_checker
    ...
    base_docker_command_args_checker = [
        ...
        "--security-opt=no-new-privileges",
        "--memory",
        f"{checker_mem_limit_kb}k",
        "--memory-swap",
        f"{checker_mem_limit_kb}k",
        "--network",
        "none",
        "-v",
        f"{str(checker_submission_dir.resolve())}:/app:ro",
        "-w",
        "/app",
    ...
    try:
        checker_process = subprocess.Popen(
            full_checker_run_cmd,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
        )
        checker_stdout, checker_stderr = checker_process.communicate(
            timeout=checker_time_limit_s + 3
        )
            ...
            if lines:
                for i_line in range(len(lines) - 1, -1, -1):
                    ...
                    if len(parts) >= 6 and parts[4].endswith("%"):
                        checker_resource_msg = f"\nChecker Resources: {lines[i_line]}"
                        break
