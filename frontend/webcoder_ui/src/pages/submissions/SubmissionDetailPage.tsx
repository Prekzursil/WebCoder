...
const SubmissionDetailPage: React.FC = () => {
	...
	const auth = useAuth();
	const [submission, setSubmission] = useState<DetailedSubmissionType | null>(
		null,
	);
	...
	const fetchDetails = async (currentSubmissionId: string) => {
		try {
			const response =
				await SubmissionService.getSubmissionDetail(currentSubmissionId);
			const data = response.data as DetailedSubmissionType;
			setSubmission(data);
			if (
				data &&
				(data.verdict === "PENDING" ||
					data.verdict === "COMPILING" ||
			...
			setError(
				err.message ||
					t(
						"error_loading_submission_detail",
	...
	useEffect(() => {
		if (submissionId && auth.token) {
			...
			setError(
				t(
					"error_auth_required_submission",
					"Authentication required to view submission details.",
				),
			);
			setLoading(false);
		}
	...
	if (!submission)
		return <p>{t("submission_not_found", "Submission not found.")}</p>;

	const problemTitle =
		submission.problem.title_i18n[i18n.language] ||
		submission.problem.title_i18n.en ||
	...
	const renderTestResult = (result: SubmissionTestResultType) => (
		...
		>
			<h4>
				{t("test_case_label", "Test Case")} #
				{result.test_case_details?.order ?? result.test_case_details?.id}
				{result.test_case_details?.is_sample &&
					` (${t("sample_label", "Sample")})`}
				{" - "} {t(`verdict_${result.verdict}`, result.verdict)}
			</h4>
			<p>
				<strong>{t("points_label", "Points")}:</strong>{" "}
				{result.test_case_details?.points}
			</p>
			{result.execution_time_ms !== null &&
				result.execution_time_ms !== undefined && (
					<p>
						<strong>{t("time_label", "Time")}:</strong>{" "}
						{result.execution_time_ms} ms
					</p>
				)}
			{result.memory_used_kb !== null &&
				result.memory_used_kb !== undefined && (
					<p>
						<strong>{t("memory_label", "Memory")}:</strong>{" "}
						{result.memory_used_kb} KB
					</p>
				)}

			{result.actual_output &&
				(result.verdict === "WA" || result.verdict === "RE") && (
					<>
						<p>
							<strong>
								{t("actual_output_label", "Actual Output (truncated)")}:
							</strong>
						</p>
						<pre
							style={{
			...
			{result.error_output &&
				(result.verdict === "RE" ||
					...
					<>
						<p>
							<strong>
								{t("error_output_label", "Error Output (truncated)")}:
							</strong>
						</p>
						<pre
							style={{
	...
	return (
		<div>
				...
				{isPolling &&
					...
					(submission.verdict === "PENDING" ||
						...
						<span style={{ fontSize: "0.8em", marginLeft: "10px" }}>
							({t("polling_status", "Polling for updates...")})
						</span>
					)}
			</h2>
			{error && !loading && (
			...
			<p>
				...
				<Link to={`/problems/${submission.problem.id}`}>{problemTitle}</Link>
			</p>
			{submission.user && (
				<p>
			...
			<p>
				<strong>{t("language_label", "Language")}:</strong>{" "}
				{submission.language}
			</p>
			<p>
				<strong>{t("verdict_label", "Verdict")}:</strong>{" "}
				{t(`verdict_${submission.verdict}`, submission.verdict)}
			</p>
			<p>
			...
			{submission.execution_time_ms !== null && (
				<p>
					<strong>
						{t("execution_time_label", "Execution Time (Overall)")}:
					</strong>{" "}
			...
			{submission.memory_used_kb !== null && (
				<p>
					<strong>{t("memory_used_label", "Memory Used (Overall)")}:</strong>{" "}
			...
			<div style={{ fontSize: "0.9em", maxWidth: "100%", overflowX: "auto" }}>
				<SyntaxHighlighter
					language={getSyntaxHighlighterLanguage(submission.language)}
					style={vscDarkPlus}
					showLineNumbers
				>
					{submission.code}
