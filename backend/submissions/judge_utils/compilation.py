...
def compile_code_in_sandbox(
    ...
    """
    print(
        f"Attempting to compile code for language: {language} in dir {submission_dir}..."
    )
    ...
    if language == "python3":
        ...
        return (
            True,
            "Python code - written to file, no compilation needed.",
            str(source_file),
        )
        ...
        if "boost_headers" in custom_libraries_allowed:
            ...
            if (
                host_boost_path and pathlib.Path(host_boost_path).exists()
            ):  # Check if path is configured and exists
                print(f"Boost headers requested. Mounting from {host_boost_path}")
                volume_mounts.extend(
                    ["-v", f"{host_boost_path}:{CONTAINER_BOOST_INCLUDE_PATH}:ro"]
                )
                gpp_command.insert(1, f"-I{CONTAINER_BOOST_INCLUDE_PATH}")
            elif host_boost_path:
                print(
                    f"Warning: Boost headers requested, but configured path '{host_boost_path}' does not exist on the host. Compilation may fail if Boost is used."
                )
            else:
                print(
                    "Warning: Boost headers requested, but JUDGE_BOOST_HEADERS_PATH is not configured in settings. Compilation may fail if Boost is used."
                )
        ...
        try:
            result = subprocess.run(
                compile_command_list, capture_output=True, text=True, timeout=30
            )
            compiler_output = f"STDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
            if result.returncode == 0 and executable_file_on_host.exists():
                return (
                    True,
                    f"C++ Compilation successful.\n{compiler_output}",
                    str(executable_file_on_host),
                )
            else:
                return (
                    False,
                    f"C++ Compilation failed (return code {result.returncode}).\n{compiler_output}",
                    None,
                )
        ...
        try:
            ...
            if result.returncode == 0 and class_file_on_host.exists():
                return (
                    True,
                    f"Java Compilation successful.\n{compiler_output}",
                    str(submission_dir),
                )
            else:
                return (
                    False,
                    f"Java Compilation failed (return code {result.returncode}).\n{compiler_output}",
                    None,
                )
        except subprocess.TimeoutExpired:
