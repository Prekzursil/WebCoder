...
const MyCreatedProblemsPage: React.FC = () => {
		...
		if (auth.isAuthenticated && auth.user?.id && auth.token) {
			...
			ProblemService.getProblems({ authorId: auth.user.id })
				...
				.catch((err: any) => {
					setError(
						err.message ||
							t("error_loading_my_problems", "Failed to load your problems."),
					);
					setLoading(false);
				});
	...
	const handleSubmitForApproval = async (problemId: number) => {
		...
		try {
			...
			setMyProblems((prev) =>
				prev.map((p) =>
					p.id === problemId ? { ...p, status: "PENDING_APPROVAL" } : p,
				),
			);
			toast.success(
				t(
					"problem_submitted_for_approval_success",
					"Problem submitted for approval!",
				),
			);
		} catch (err: any) {
			toast.error(
								...
								<td>
									...
									{problem.status === "PRIVATE" &&
										problem.verifier_feedback && (
											...
											>
												<em>
													{t("verifier_feedback_label", "Feedback")}:{" "}
													{problem.verifier_feedback}
												</em>
											</p>
										)}
